...
class counterpartsHandler(RedisSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pageSize = requests.post(url=count_data["url"],
                                      data=count_data["formdata"],
        ...
        if self.r.get(self.crawled_count_key) and self.r.get(self.dequeued_count_key):
            if self.r.llen(self.redis_key) == 0 and self.r.get(self.crawled_count_key) >= self.r.get(self.dequeued_count_key):
                ...
                self.crawler.engine.close_spider(
                    spider=self, reason="Queue is empty, the spider closes")
    ...
    def make_request_from_data(self, data):
        ...
        ctp["formdata"]["PageSize"] = self.pageSize
