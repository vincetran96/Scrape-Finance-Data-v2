...
class balanceSheetCafeFHandler(fadRedisCafeFSpider):
    ...
    def next_requests(self):
        ...
        self.logger.info(f'Total requests supposed to process: {self.r.get(self.ticker_page_count_key)}')

        ### Close spider if corpAZ_cafef is closed AND none in queue AND spider is idling
        ### Log the requests with errors, then delete all keys related to this Spider
    ...
    def parse(self, response):
        ...
        if response:
            ...
            try:
                ...
                ### NOTE: In a GUI browser, there's an element <tbody> under <table>,
