...
while true; do
    ...
    while true; do
        ...
        if [[ "$massyn" == "y" || "$massyn" == "Y" ]]; then
            ...
            while true; do
                read -p "Do you wish to start mass scraping now? [y] " startmassy
            ...
            while true; do
                ...
                if [[ "$bityn" == "y" || "$bityn" == "Y" ]]; then
                    ...
                    echo "Start scraping $biz_ind_ids, report type(s) $report_type, report term(s) $report_term"
                    scrapy crawl financeInfoOnDemand -a biz_ind_ids=$biz_ind_ids -a report_type=$report_type -a report_term=$report_term
                elif [[ "$bityn" == "n" || "$bityn" == "N" ]]; then
                    read -p "Enter a ticker, or a tickers list in the form of ticker_1,ticker2,...:" ticker
                    read -p "Enter report type id: " report_type
                    read -p "Enter report term id: " report_term
                    read -p "Enter page number: " page
                    if [[ "$page" == "" ]]; then
                        echo "Page number defaulted to 1"
                        echo "Start scraping $ticker, report type(s) $report_type, report term(s) $report_term, page 1"
                        scrapy crawl financeInfoOnDemand -a ticker=$ticker -a report_type=$report_type -a report_term=$report_term -a page=1
                    elif ! [[ "$page" =~ ^[0-9]+$ ]]; then
                        echo "Please enter a positive integer only."
                    else
                        echo "Start scraping $ticker, report type(s) $report_type, report term(s) $report_term, page $page"
                        scrapy crawl financeInfoOnDemand -a biz_ind_ids=$biz_ind_ids -a report_type=$report_type -a report_term=$report_term -a page=$page
                    fi
                else
                    echo "Please provide your answer."
                fi        
